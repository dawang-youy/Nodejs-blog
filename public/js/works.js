...
layui.use(["element"], () => {
  ...
  window.onload = () => {
    ...
    function renderArt(data) {
        ...
        artList.forEach((v, i) => {
          str += `<li>
                    ...
                    <div class="list-text">
                      ...
                      <p>
                        <a href=/article/${v._id} title="阅读" target="_blank">
                          阅读全文
        ...
        // layui组件的加载
        window.pageObj.time && layui.use(["element", "laypage"], () => {
          let element = layui.element
          ...
          laypage.render({
            ...
            limit: 10,
            groups: 3,
            curr: location.pathname.replace("/api/works/", ""),
            jump(obj, f){
              $("#laypage a").each((i, v) => {
              ...
              getData("/api/works/"+obj.curr).then(data => renderArt(data));
            }
          });
          window.pageObj = {
            time:false,
    ...
    //渲染 文章列表
    getData("/api/works/1").then(data => renderArt(data));
    //渲染 网站公告和博客简介
    getData("/api/notice").then(data => renderNotice(data));
    //渲染 最近更新
    getData("/api/newlist?type=works").then(data => renderNew(data));
    //渲染 热度排行
